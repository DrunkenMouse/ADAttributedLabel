...
NSString * const kADTextRunAttributedName = @"ADTextRunAttributedName";
...
- (BOOL)enumerateRunRectContainPoint:(CGPoint)point viewHeight:(CGFloat)viewHeight successBlock:(void (^)(id<ADTextStorageProtocol>))successBlock;

- (BOOL)enumerateLinkRectContainPoint:(CGPoint)point viewHeight:(CGFloat)viewHeight successBlock:(void (^)(id<ADLinkStorageProtocol>linkStorage))successBlock;

@end

@interface ADAttributedLabel()<UIGestureRecognizerDelegate> {
...
@implementation ADAttributedLabel
...
-(instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        [self setupProperty];
        _textContainer = [[ADTextContainer alloc] init];
    }
    return self;
}
...
-(instancetype)initWithTextContainer:(ADTextContainer *)textContainer {
    if (self = [super init]) {
        [self setupProperty];
...
- (void)setTextContainer:(ADTextContainer *)attStringCreater {
    _textContainer = attStringCreater;
    [self resetAllAttributed];
    _preferredMaxLayoutWidth = attStringCreater.textWidth;
    [self invalidateIntrinsicContentSize];//当影响内在内容大小的某些更改时调用此函数。 否则UIKit不会注意到它改变了。
    [self setNeedsDisplay];
}

- (void)setDelegate:(id<ADAttributedLabelDelegate>)delegate {
         ...
         context: (CGContextRef)context
{
    if (_textContainer.numberOfLines > 0)
    {
        ...
        //kCTLineBreakByTruncatingTail:显示每一行，以便开始适合容器，并且丢失的文本由某种省略号字形指示。
        ...
        for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++)
        {
            ...
            if (lineIndex == numberOfLines - 1 && truncateLastLine)
            {
                ...
                {
                    CTLineTruncationType truncationType = kCTLineTruncationEnd;//设置分割类型
                    NSUInteger truncationAttributePosition = lastLineRange.location + lastLineRange.length - 1;//设置分割位置
                    
                    NSDictionary *tokenAttributes = [attributedString attributesAtIndex:truncationAttributePosition effectiveRange:NULL];//设置分割范围
                    NSAttributedString *tokenString = [[NSAttributedString alloc] initWithString:kEllipsesCharacter attributes:tokenAttributes];//设置用于分割的字符串
                    CTLineRef truncationToken = CTLineCreateWithAttributedString((CFAttributedStringRef)tokenString);//根据分割的字符串生成Line
                    
                    NSMutableAttributedString *truncationString = [[attributedString attributedSubstringFromRange:NSMakeRange(lastLineRange.location, lastLineRange.length)] mutableCopy];//获取最后一行的字符串
                    
                    if (lastLineRange.length > 0)
                    {
                        // Remove last token
                        [truncationString deleteCharactersInRange:NSMakeRange(lastLineRange.length - 1, 1)];//字符串大于0就删除最后一个字符
                    }
                    [truncationString appendAttributedString:tokenString];
                    //拼接省略的字符串
                    
                    CTLineRef truncationLine = CTLineCreateWithAttributedString((CFAttributedStringRef)truncationString);//根据拼接好的字符串生成Line
                    CTLineRef truncatedLine = CTLineCreateTruncatedLine(truncationLine, rect.size.width, truncationType, truncationToken);//根据最后一行，行的宽度，分割类型(END)，分割的Line生成新的Line
                    
                    if (!truncatedLine)
                    {
                        // If the line is not as wide as the truncationToken, truncatedLine is NULL
                        //如果该行不像truncationToken一样宽，truncatedLine为NULL
            ...
            if(shouldDrawLine)
            {
                CTLineDraw(line, context);//将行绘制到上下文，Line中就有需要绘制的文本内容
            }
        }
    }
    else
...
{
    // draw storage TYTextContainer里调用，传递需要绘制的大小与仓库
    [_textContainer enumerateDrawRectDictionaryUsingBlock:^(id<ADDrawStorageProtocol> drawStorage, CGRect rect) {
        if ([drawStorage conformsToProtocol:@protocol(ADViewStorageProtocol) ]) {
            
            //要绘制的OwenrView，就是添加的控件父类。通过addSubview添加按钮等
            [(id<ADViewStorageProtocol>)drawStorage setOwnerView:self];//画Image与View
        }
        rect = UIEdgeInsetsInsetRect(rect,drawStorage.margin);
        [drawStorage drawStorageWithRect:rect];
    }];
    
    if ([_textContainer existRunRectDictionary]) {//设置点击与长按事件
...
- (void)singleTap:(UITapGestureRecognizer *)sender {
    ...
    [_textContainer enumerateRunRectContainPoint:point viewHeight:CGRectGetHeight(self.frame) successBlock:^(id<ADTextStorageProtocol> textStorage) {
...
-(void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if ([_textContainer existLinkRectDictionary]) {
        ...
        __typeof (self) __weak weakSelf = self;
        [_textContainer enumerateLinkRectContainPoint:point viewHeight:CGRectGetHeight(self.frame) successBlock:^(id<ADLinkStorageProtocol> linkStorage) {
            NSRange curClickLinkRange = linkStorage.realRange;
            [weakSelf setHighlightedLinkWithSavaLinkColor:(linkStorage.textColor ? linkStorage.textColor : weakSelf.textContainer.linkColor) linkRange:curClickLinkRange];
            found = YES;
        }];
    }
    if (!found) {
...
-(void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (isUnderClickLink) {
        if (!NSEqualRanges(curClickLinkRange, _clickLinkRange)) {
            ...
            [self setHighlightedLinkWithSavaLinkColor:saveLinkColor linkRange:curClickLinkRange];
        }
...
// 设置高亮链接
- (void)setHighlightedLinkWithSavaLinkColor:(UIColor *)saveLinkColor linkRange:(NSRange)linkRange {
...
-(void)setFrameWithOrigin:(CGPoint)origin Width:(CGFloat)width {
    
    //获得高度
    int height = [self getHeightWithWidth:width];
    
    //设置frame
    [self setFrame:CGRectMake(origin.x, origin.y, width, height)];
}
...
@implementation ADAttributedLabel (AppendAttributedString)
...
@implementation ADAttributedLabel (Link)
...
@implementation ADAttributedLabel (UIImage)
...
{
    [self appendImageWithName:imageName size:size alignment:ADDrawAlignmentTop];
}
...
@implementation ADAttributedLabel (UIView)
