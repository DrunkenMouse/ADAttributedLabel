...
typedef NS_ENUM(NSUInteger, ADVerticalAlignment) {

    ADVerticalAlignmentTop,
    ADVerticalAlignmentCenter,
    ADVerticalAlignmentBottom,
};

@class ADAttributedLabel;
@protocol ADAttributedLabelDelegate <NSObject>
@optional
// 点击代理
...
@end
/**
 *  ADAttributedLabel 属性文本 支持图文混排显示，支持添加image和UIView，支持自定义排版
 */
@interface ADAttributedLabel : UIView

@property (nonatomic, assign) id<ADAttributedLabelDelegate> delegate;
@property (nonatomic, copy) NSString *text;
@property (nonatomic, strong) UIColor *textColor;//文字颜色
@property (nonatomic, strong) UIFont *font;//文字大小
@property (nonatomic, assign) NSInteger numberOfLines;//次数
...
@property (nonatomic, assign) CTLineBreakMode lineBreakMode;//换行模式kCTLineBreakByCharWrapping
@property (nonatomic, assign) ADVerticalAlignment verticalAlignment;//垂直对齐方式 默认是向上对齐
...
@interface ADAttributedLabel(AppendAttributedString)
/**
 *  追加(添加到最后) 普通文本
 *
 *  @param text 普通文本
...
@interface ADAttributedLabel(Link)
/**
...
@interface ADAttributedLabel (UIImage)
